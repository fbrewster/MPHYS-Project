...
function lungConvexHull()--Makes a convex hull from the delineation of both lungs
  ...
  local lungs = wm.Delineation.Heart or wm.Delineation.heart --get heart delin, refered to as 
...
function bubbleCent (inSc,cHull)--finds the connected pixels and their centres
  ...
  for i = 1,nBubbles do
    ...
    if volTemp < 2 and volTemp>0.001 then--check to see if volumes are too small or too large
...
function getCal(scanNum)--find calcifications and write them out to a xdr file
  ...
  AVS:FIELD_OPS(cHull.Data,cHull.Data,5,AVS.FIELD_OPS_Closing)
  AVS:FIELD_OPS(cHull.Data, f, 1, AVS.FIELD_OPS_SignedDist, false, 
    2.5*(math.floor(math.abs(cHull.Data:pixelsize(0).value)*1000 + 0.5)), 
    (math.floor(math.abs(cHull.Data:pixelsize(1).value)*1000 + 0.5)), 
    2.5*(math.floor(math.abs(cHull.Data:pixelsize(2).value)*1000 + 0.5)))
  AVS:FIELD_DIVC(f, f, 1000)
  AVS:FIELD_THRESHOLD(f,cHull.Data, -1000,0.85)
  f:clear()
  ...
  local lowThresh = hist:percentile(99.9).value--find 99th percentile pixel value
  if lowThresh<1275 then
    lowThresh=hist:percentile(99.95).value
    if lowThresh<1275 then
      lowThresh=1275
    end
  end
  ...
  AVS:FIELD_THRESHOLD(tempScan, shcHull.Data, 137)--shrink it
  scans[1] = shcHull
  ...
  for i=1,#cents do--floodfill from centre of each bubble
    ...
    local floodThresh = bubHist:percentile(87.5).value--find 80th percentile pixel value
