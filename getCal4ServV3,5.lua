...
function lungConvexHull(orign)--Makes a convex hull from the delineation of both lungs
  ...
  AVS:TT_CLEAN_TRIANGLES(chIndex, chIndex)--clean ch triangles
  local strBuffer = String:new()
  AVS:TT_CLEAN_VERTICES(lungs.dots, chIndex, chDots, chIndex, strBuffer)--clean ch vertices
...
function intersect(s1,s2)--find if s1 is fully in s2
  local vol1=s1:volume().value--volume scan 1
  local s1s2 = Scan:new()
  s1s2:setup()
  AVS:FIELD_AND(s1.Data,s2.Data,s1s2.Data)--get only part of s1 that is also in s2
  local vol2=s1s2:volume().value--get new volume
  if vol1==vol2 then--if all of s1 is not in s2
    return false
  else
    return true
  end
...
function TScan:objectness(sigMin,sigMax,nOfSigs,M)--run objectness exe
  self:write_nifty([[C:\temp\temp.nii]],true)--write out nifty with adjust
  ...
  inScan:setup()
  inScan:read_nifty([[C:\temp\vesselness.nii]])--read in exe output
  return inScan
end
...
function getCal()--find calcifications and write them out to a xdr file
  ...
  local orignEnhanced = Scan:new()--start with original scan
  orignEnhanced = wm.Scan[1]:copy()
  
  local orign = Scan:new()--copy original into envirnment
  orign = wm.Scan[1]:copy()
  
  local clip = Scan:new()
  clip:setup()
  
  AVS:FIELD_CLIP(orignEnhanced.data, clip.Data, 1135, 1650)--clip original
  
  local vess = clip:objectness(3,3,1,1)--run vessellness at scale 3
  AVS:FIELD_OPS(vess.Data, vess.Data, 3, AVS.FIELD_OPS_Smooth)
  
  local plate = clip:objectness(2.5,2.5,1,2)--run plateness at scale 2.5
  AVS:FIELD_OPS(plate.Data, plate.Data, 3, AVS.FIELD_OPS_Smooth)
  
  local origF = clip:copy()--convert clip to float
  origF.data:tofloat()
  ...
  AVS:FIELD_MULC(vess.data,toMultV,0.75)--vess x 0.75
  AVS:FIELD_MULC(plate.data,toMultP,0.5)--plate x 0.5
  toMultV:tofloat()
  toMultP:tofloat()
  AVS:FIELD_ADDC(toMultP,toMultP,(1-0.5))--normalise to run up to 1
  AVS:FIELD_ADDC(toMultV,toMultV,(1-0.75))
  AVS:FIELD_ADD(toMultP,toMultV,toMult)--add vess to plate
  
  AVS:FIELD_MUL(toMult,origF.data,mult)--multiply clipped by enhancement
  local enhanced = vess:copy()--get adjust and transform of vessellness
  mult:toshort()--convert field to short
  enhanced.data=mult
  ...
  --Go to vess & plate adjust and transform
  mTrans=orignEnhanced.Transform
  mAdjust=orignEnhanced.Adjust

  local lowThresh = 1250*1.5--thresh off 1250 original pixel value, enhaced by 1.5
  local highThresh = hist:max().value--find largest pixel value

  -------Find seeds in enhanced image-------
  
  --threshold
  local threshed = Scan:new()
  threshed:setup()
  AVS:FIELD_THRESHOLD( orignEnhanced.Data, threshed.Data, lowThresh, highThresh )--

  --Smooth and close
  local smoothed = Scan:new()
  smoothed:setup()
  AVS:FIELD_OPS( threshed.Data, smoothed.Data, 1, AVS.FIELD_OPS_Smooth )--Smoothing filter
  ...
  --shrink lung ch without spine cut out
  ...
  --Move back to original transform and adjust
  mAdjust = wm.Scan[1].Adjust:copy()
  mTrans = wm.Scan[1].Transform:copy()

  -------Fill seeds on original image-------
  ...
  for i=1,#cents do--floodfill from centre of each bubble
    ...
    --Get centre point in original frame
  ...
  totBubFlood:setup()
  AVS:FIELD_THRESHOLD( flood.Data, totBubFlood.Data, 4000,5000)--take only flooded pixels
  AVS:FIELD_OPS(totBubFlood.Data, totBubFlood.Data, 2, AVS.FIELD_OPS_Closing)--closing
  ...
  for i=1,bubFlood.Data:max().value do
    ...
    if thisBub.Data:max().value>0 and thisBub:volume().value<maxVol and thisBub:volume().value>0.01 then--if the bubble exists and isn't too large
      local cross=intersect(thisBub:copy(),shcHull:copy())--check if it crosses shrunk lungs
      if not cross then
